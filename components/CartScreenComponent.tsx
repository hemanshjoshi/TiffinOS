...
import { ArrowLeft, Ticket, ChevronRight, X } from 'lucide-react-native';
...
export default function CartScreenComponent({ isTab = false }: CartScreenProps) {
    ...
    setTimeout(async () => {
      try {
        const orderData = {
          ...
          payment_method: 'CASH_ON_DELIVERY', // Placeholder for now
  ...
  if (items.length === 0) {
     return (
        <SafeAreaView style={styles.emptyContainer} edges={isTab ? ['top'] : ['top', 'left', 'right']}>
            <Text style={styles.emptyText}>Your cart is empty</Text>
            {isTab ? (
                 <TouchableOpacity onPress={() => router.push('/(tabs)/home')} style={styles.browseButton}>
  ...
  return (
    <SafeAreaView style={styles.container} edges={isTab ? ['top'] : ['top', 'left', 'right']}>
          ...
          >
             {isProcessingPayment ? (
               <View style={{ flex: 1, alignItems: 'center' }}>
                 <Text style={styles.payText}>Processing Order...</Text>
               </View>
             ) : (
               <>
                 <View>
